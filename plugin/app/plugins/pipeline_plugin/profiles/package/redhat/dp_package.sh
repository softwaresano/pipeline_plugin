...
function createChangeLog(){
   ...
   if [ "`cat $modifiedSpecFile|egrep \"^%changelog\"`" == "" ]; then
      if [ "$scmtype" == "git" ]; then
         ...
         git log ${current_version}/KO..HEAD --format="%ad - %aE %h %s %b"|grep "Merge pull request #"|awk -v current_version="$current_version" '{print "* "$1" "$2" "$3" "$5" - "$8" - " current_version;print "  - "$15"\n  "}'|head -1 >> $modifiedSpecFile
