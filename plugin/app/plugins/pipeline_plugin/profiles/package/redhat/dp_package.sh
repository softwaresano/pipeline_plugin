...
function createRPM(){
   ...
   rpmBuildCommand="rpmbuild -v --clean ${CUSTOM_RPM_MACROS} --define 'dp_package 'true --define 'os_release '$(dp_os_release.sh) --define 'versionModule '$versionModule \
...
function buildRPMS(){
   ...
   for specFile in $(find . -maxdepth 7 -type f -name "*.spec"  -not -path "./target/*" -not -path "./external/*" -not -path "./.git/*" -not -path "./.scannerwork/*"|sort); do
      createRPM "$specFile"
      if [ "$?" == "0" ]; then
